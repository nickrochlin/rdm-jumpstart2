...
```{r}
# Explore the unique values in the feelRushed column
...
```{r}
# Filter rows where the sleep duration is less than 600
...
```{r}
# Filter rows using the | (or) operator
...
```{r}
# Select columns for time perceptions and time spent on work/study
...
```{r}
# Filter rows: select respondents in age group "30-39" and who are Married
...
```{r}
# Select columns that capture social time
