...
<h4 style="text-align:center;">
  <strong>
    Saving output is a crucial part of responsible research data management and directly supports the FAIR principles: making data <em>Findable, Accessible, Interoperable, and Reusable</em>.
  </strong>
</h4>
...
#### Save Datasets as CSV or TSV for __Interoperability__ and __Reusability__

Now, let's save this dataset into a CSV (Comma-Separated Values) or TSV (Tab-Separated Values) file for sharing or reuse. Both are __non-proprietary__ formats that promote **interoperability** and **accessibility**.

TSV is more versatile and enhances **reusability** due to:
...
:::note
Both `write_csv()`  and `write_tsv()` (from the tidyverse) is a fast, UTF-8 CSV writer that by default skips row names and matches `read_csv()` (also a tidyverse function) conventions, while base R’s `write.csv()` is slower and writes a row-names column unless you turn it off.
:::

The `js_data` object contains the dataset we used, transformed, and modified up to now. In this session, we will assume this is our final dataset to deposit into a repository such as OSF or Borealis. Saving in `.csv` or `.tsv` makes sure our files are open and usable across platforms, easily imported into other programming languages, databases, or spreadsheet software.
...
#### Choosing Between Proprietary and Non-Proprietary Formats

Following the use of non-proprietary `.csv` and `.tsv` formats for broad sharing, consider proprietary formats like `.rds` or `.RData` only when your data is intended for use within the sepecific ecosystem (in this case, it is R ecosystem). 

- **Non-proprietary formats** (e.g., `.csv`, `.tsv`): Ensure **interoperability** across diverse systems and users, as discussed earlier, making data accessible to all.
- **Proprietary formats** (e.g., `.rds`, `.RData`): Optimize **reusability** for R users by preserving R-specific data structures, but limit accessibility outside R.
...
Here, we can save the `js_data` in `.Rdata` format by using the `save()` function:
...
#### Types of Data Dictionaries: Human-Readable vs. Machine-Readable
...
### Exporting the Full Document as PDF or HTML
...
#### 1. Add (or confirm) your YAML header
...
#### 2. Install a LaTeX distribution (for PDF)
...
TinyTeX is lightweight and will automatically pull in any missing LaTeX packages as you knit.
