...
| `recode()` | Change data format of variables or re-categorizing certain values in a variable as some other value.  Often used in conjunction with `mutate` (see below for example). |
...
| `is.numeric` | Check to see is the data is of a numeric class, which will return a TRUE/FALSE response. `is.numeric(data)` |
