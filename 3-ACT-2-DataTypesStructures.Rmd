...
This section covers how data is organized in R. Understanding how your data is organized and being able to adjust that organization to match your research needs is an important part of research data management. For example, you might need your data to be in a certain structure to run certain statistical analyses or you may need to transform variables into versions that are more easily understood. This section covers four main topics: 

1. What data types exist in R (i.e., is your data composed of numbers, words, or both) 
2. Dealing with missing values 
3. Re-coding existing data into another type of data
4. What data structures exist in R (i.e., do you have one long list of elements, a spreadsheet, or something else)
...
Boolean and logical operators can be used to compare values or create logical statements. These are a bit different from the rest of the data types, because you usually don't have these operators within a dataset. However, they are very useful for understanding, checking, and managing your data. 

You might have used Boolean characters to search databases in the past (e.g., searching in journal databases for articles with certain keywords). They work similarly in R. Boolean (or comparison) operators allow you to check conditions within your dataset. These return `TRUE` or `FALSE` based on whether the condition is met. These functions also can be useful if you want to filter for a certain value in the data (e.g., sub-set only women in a dataset) or modify how data is grouped (e.g., you want to re-label everyone over 6 feet as "tall"). Logical operators allow you to filter data based on multiple conditions.
...
In order to check and modify data types we suggest using the `dplyr` package. `dplyr` is a powerful R package designed to simplify data manipulation tasks. It provides a set of intuitive functions that make filtering, selecting, and transforming data frames (we will talk about this data structure soon) more readable and efficient. It is the go to package for data set organization. We will use just a few of the functions of this package in this course, but there are many more applications of `dplyr` you can explore. 
...
First we load our data and the dplyr package (remember to install it first if you haven't yet). Just to reacquaint ourselves with our data, use the `head` function. 
...
You can check the data type of a specific variable with the "class" function,
...
:::walkthrough
  - This code is telling R to `recode()` the variable "province" from the dataset "data" into a new variable called "province_char" (you could call it anything, but make sure it is a meaningful name).

  - The list outlines how each value of the current "province" variable should be recoded for the new "province_char" variable. 
  
  - The `mutate()` portion tells R to create a new column called "province_char". 
  
  - Before the `mutate()` function this code tells R to take the "data" object (i.e., the dataset) "and then" (i.e., the pipe function) apply the following functions. 
  ...
  - Lastly, all of this work needs to be assigned back to our dataset "data" in order to save it using the `<-` symbol. 
:::
