...
First we load  the `dplyr` package (remember to install it first if you haven't yet). You can find more information about `dplyr` on the <a href="https://dplyr.tidyverse.org/index.html">tidyverse website</a>. If you have installed the `tidyverse` package, then `dplyr` was also included. However, it is best practice to just load in the libraries you need (i.e., just `dplyr` and not all of the tidyverse packages) to save space.
...
However, maybe we would prefer to have province acronyms (i.e., character data format) rather than the integer representation of them. We can re-code variables using the dplyr functions `|>` and `mutate()`. The possible integer values for the province variable, and what province they represent can be found in the dataset codebook. 
...
If your data contains NA values, sometimes functions will give error messages due to incomplete data. For example, if you try to get the mean of education level (eduLevel), R will not provide one because it can't calculate a mean when there are missing values. Note: getting a mean for education level, which is a categorical variable, is not a meaningful number. However, it is used here to illistratue this point, given that the continuous numeric variables in this dataset do not contain missing values. 
...
Let's learn more about this function to see what happened. 
```{R}
help(mean)
```
We can see that the default for `mean` is to have `na.rm = FALSE`.
...
a) The 10 most popular movies this year (10 data points)

b) The names of the leading actor from each of the 10 most popular movies this year (10 data points total)

c) A collection of all of the actors in each of the 10 most popular movies this year (100 data points total)

d) A combination of a and b

e) A combination of a, b, and c

f) The data we have been working with in this workshop
:::
