...
```{r}
js_data <- js_data |> mutate(province_char = recode_factor(js_data$province, 
                            "10" = "NL", 
                            "11" = "PEI", 
                            "12" = "NS",
                            "13" = "NB", 
...
:::walkthrough
  - This code is telling R to `recode_factor()` the variable "province" from the dataset "data" into a new variable called "province_char" (you could call it anything, but make sure it is a meaningful name). We are using `recode_factor()` instead of just `recode()` to keep the data in factor structure. 
...
```{r class.source = 'fold-hide', results = 'hide'}
js_data<-js_data |> mutate(maritalStat_char = recode_factor(js_data$maritalStat, 
                            "1" = "Married", 
                            "2" = "Living common-law", 
                            "3" = "Widowed",
                            "4" = "Separated", 
...
class(js_data$maritalStat_char)
```
...
```{r class.source = 'fold-hide', results = 'hide'}
js_data <- js_data |> mutate(eduLevel_char = recode_factor(js_data$eduLevel,
