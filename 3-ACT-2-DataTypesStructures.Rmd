...
You may see mention of tibbles, which are an updated version of data frames and part of the tidyverse package. They are a bit more user-friendly version of tabular data. 
...
Factor structure is particularly useful for statistical modeling. Sometimevs you might need to change an object into factor structure, so you can use the function `as.factor()` to do so. 
...
First we load our data and the `dplyr` package (remember to install it first if you haven't yet). If you have installed the `tidyverse` package, then `dplyr` was also included. Just to reacquaint ourselves with our data, use the `head` function. 
...
If you want to know the total number of rows (i.e., participants) with no missing data you can take the `complete.cases()` function and wrap it in the `nrow()` funciton. 
